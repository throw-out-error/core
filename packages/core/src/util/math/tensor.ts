...
> {
    ...
    public static get VECTOR_4_ZERO(): Tensor<Vector> {
        return Tensor.zeros([3]);
    }
    ...
    constructor(
        ...
        this.data = type.from(
            (((data as number[]).flat(Infinity) ?? [data]) as unknown) as T
        );
    ...
    dot(m: Tensor): number {
        let result = 0;
        this.forEach(
            (_: number, i: number) => (result += this.data[i] * m.data[i])
        );
        return result;
    }
