import { Controller } from "@toes/core/interfaces";
...
export abstract class ContextCreator {
    ...
    public createContext<T extends unknown[] = any, R extends unknown[] = any>(
        ...
        const classMetadata = this.reflectClassMetadata<T>(
            instance,
            metadataKey
        );
        const methodMetadata = this.reflectMethodMetadata<T>(
            callback,
            metadataKey
        );
        return [
