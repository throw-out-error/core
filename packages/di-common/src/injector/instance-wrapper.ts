import { Provider, Scope, Type } from "@toes/core";
import {
    ClassProvider,
    FactoryProvider,
    ValueProvider,
} from "@toes/core/interfaces";
import { randomStringGenerator } from "@toes/core/utils/random-string-generator.util";
import { isNil, isUndefined } from "@toes/core/utils/shared.utils";
...
export class InstanceWrapper<T = any> {
    ...
    constructor(
        metadata: Partial<InstanceWrapper<T>> &
            Partial<InstancePerContext<T>> = {}
    ) {
    ...
    public isDependencyTreeStatic(lookupRegistry: string[] = []): boolean {
        ...
        isStatic =
            isStatic &&
            this.isWrapperListStatic(propertiesHosts, lookupRegistry);
    ...
    public isExplicitlyRequested(
        ...
        return (
            ...
            contextId !== STATIC_CONTEXT &&
            (isSelfRequested ||
                (inquirer && inquirer.scope === Scope.TRANSIENT))
        );
    }
