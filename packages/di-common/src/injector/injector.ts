...
} from "@toes/core/constants";
import { Controller } from "@toes/core/interfaces/controllers/controller.interface";
import { Injectable } from "@toes/core/interfaces/injectable.interface";
...
import {
    ...
    isString,
    isUndefined,
} from "@toes/core/utils/shared.utils";
...
export class Injector {
    ...
    public async loadCtorMetadata(
        ...
        return hosts.map(
            (item) =>
                item.getInstanceByContextId(contextId, inquirerId).instance
        );
    }
    ...
    public async loadPropertiesMetadata(
        ...
        return dependenciesHosts.map(({ key, host }) => ({
            key,
            name: key,
            instance: host.getInstanceByContextId(contextId, inquirerId)
                .instance,
        }));
    }
