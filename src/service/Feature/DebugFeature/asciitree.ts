...
export function drawTree<T>(
    ...
    const insert = (txt: string, onLine: number, position: number) => {
        ...
        if (position < 0) {
            throw new Error(
                `Trying to insert "${txt}" at negative position(${position}).`
            );
        }

        if (position < length) {
            throw new Error(
                `Trying to insert "${txt}" at position(${position}) less then length(${length}).`
            );
        }
    ...
    const drawNode = (tree: T, onLine: number, position: number) => {
        ...
        if (nodes && nodes.length !== 0) {
            ...
            if (nodes.length === 1) {
                ...
                foundedPadding = findPadding(
                    title,
                    onLine + 2,
                    position - offset
                );

                nodePadding = drawNode(
                    node,
                    onLine + 2,
