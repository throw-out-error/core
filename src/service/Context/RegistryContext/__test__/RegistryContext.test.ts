...
describe("createFeatureFactoryRegistryContext", () => {
    it("Can register map", () => {
        const context = createFeatureFactoryRegistryContext(
            new ServiceFunctionReferenceContainer()
        );
    ...
    it("Can register to map", async () => {
        ...
        const context = createFeatureFactoryRegistryContext<{
            MyRegister: RegistryMap<number>;
        }>(container);
        context.registers.MyRegister(["foo", () => 1]);
        await container.build();
        const registry: RegistryMap<number> = (container.getService as (
            s
        ) => () => RegistryMap<number>)("MyRegister")();
    ...
    it("Can register to list", async () => {
        ...
        const context = createFeatureFactoryRegistryContext<{
            MyRegister: RegistryList<number>;
        }>(container);
        context.registers.MyRegister(() => 1);
        await container.build();
        const registry: RegistryList<number> = (container.getService as (
            s
        ) => () => RegistryList<number>)("MyRegister")();
