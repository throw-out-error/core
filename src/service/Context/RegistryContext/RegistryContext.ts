...
import {
    createFeatureFactoryRegistrySetContext,
    RegistrySetContext,
} from "./RegistrySetContext";
...
export type InferRegisters<T> = InferListArgumentRegisters<T> &
    InferMapArgumentsRegisters<T>;

export interface RegistryContext<T>
    extends RegistryListContext<T>,
 ...
 */
export const registerToListOrSet = <Services, T>(
    container: ServiceFunctionReferenceContainerInterface,
    registry: ServiceTag,
    ...items: RegisterListArguments<Services, T>
) => {
    const serviceFactories = getServices<Services, T>(
        container.getService as any
    )(...items);
    container.override(
        new Override<RegistryList<any> | RegistrySet<any>>({
            tag: registry,
            override: (service) => {
                return service().register(
                    ...[...serviceOfServiceFactories(serviceFactories)]
                );
            },
        })
    );
...
export const registersTo = (
    ...
    return (...args: any) => {
        ...
        const first:
            | RegisterMapArgument<unknown, unknown, unknown>
            | RegisterListArgument<unknown, unknown> = args[0];
...
export const createFeatureFactoryRegisterContext = (
    container: ServiceFunctionReferenceContainerInterface
) =>
    fromPairs(
        container
            .references()
            .taggedPairs()
            .map(([name]) => [name, registersTo(container, name)])
    );
