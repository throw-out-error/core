...
export const createFeatureFactoryAsyncContext = ({
    container,
}: InternalContextContext): AsyncContext => ({
    synchronize: wrapReturnAsReference(
        <T extends SF>(sf: () => Promise<T>): T => {
            return container.add(
                new AsyncServiceFunctionReference(
                    sf,
