...
it("Can merge features", async () => {
    ...
    const context = createFeatureFactoryContext<unknown, unknown>({
        container,
...
it("Merged services can use default services", async () => {
    const spy = jest.fn();
    const myFeature: FF<unknown, SetupFeatureServices> = ({
        registers: { setupCallbacks },
    }) => ({
        ...setupCallbacks(() => spy),
    });
    await triviality()
...
it("Can merge multiple times", async () => {
    ...
    const {
        ...
        users1: u1,
        users2: u2,
    } = await triviality().add(myFeature).build();
...
it("Can merge multiple services", async () => {
    ...
    interface MyFeature extends F1, F2, F3 unknown

    const myFeature: FF<MyFeature, unknown> = ({ merge }) => {
        return merge(f1).with(f2).with(f3).all();
    };

    const { one, two, three } = await triviality().add(myFeature).build();
...
it("Can override merged services", async () => {
    ...
    interface MyFeature extends Orginal, F2, F3 unknown

    const myFeature: FF<MyFeature, unknown> = ({ merge }) => {
        return merge(orginalFeature).with(f2).with(f3).all();
    };

    const myFeatureOverride: FF<unknown, Orginal> = ({ override: { one: o } }) => {
        return {
            ...o(() => 10),
        };
    };
...
it("Can directly fetch all new services", async () => {
    ...
    const johnUserFeature: FF<unknown, UserServices> = ({
        registers: { users },
    }) => ({
        ...users(() => ({ name: "John" })),
    });

    const JaneUserFeature: FF<unknown, UserServices> = ({
        registers: { users },
    }) => ({
        ...users(() => ({ name: "Jane" })),
    });
    ...
    const myFeature: FF<MyFeatureServices, unknown> = ({ merge }) => {
        const { users } = merge(userFeature)
            .with(johnUserFeature)
            .with(JaneUserFeature)
            .services("users");
        return {
            users,
    ...
    const { names } = await triviality().add(myFeature).build();
