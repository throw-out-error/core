...
export const createFeatureFactoryOverrideContext = (
    ...
    override: fromPairs(
        container
            ...
            .map(([serviceName]) => [
                serviceName,
                overrideBy(container, serviceName),
            ])
    ),
});
...
export type Overrides<T> = {
    [K in keyof T]: (overrideWith: OverrideWith<T[K]>) => unknown;
};
...
export const overrideBy = curry(
    (
        ...
        overrideWith: OverrideWith<never>
    ): unknown => {
        container.override({
