...
export const assetServiceFactory = (sf: unknown): sf is SF | unknown => {
    if (typeof sf !== "function") {
        throw new Error("ServiceFactory is not a function");
    }
    if (sf.length > 0) {
        throw new Error(
            `ServiceFactory '${sf.name}' cannot have unknown arguments`
        );
    }
    return true;
};
...
export const serviceOfServiceFactories = <T>(
    factories: ServiceFactory<T>[]
): T[] => map(serviceOfServiceFactory, factories);
...
export const assertServiceTag = (
    tag: ServiceTag | unknown
): tag is ServiceTag | unknown => {
...
export const isServiceTag = (target: unknown): target is ServiceTag =>
    typeof target === "string";

export type ServiceFactoryByTag<T = unknown> = (tag: ServiceTag) => SF<T>;

export type ServicesOfType<Services, TType> = {
    [K in keyof Services]: Services[K] extends TType ? Services[K] : unknown;
};

export type ServiceKeysOfType<Services, TType> = keyof ServicesOfType<
    Services,
    TType
>;

export const AllAsServiceFactory = <T>(instances: T[]): SF<T>[] =>
    instances.map(always);
