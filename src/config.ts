...
const parse = (key: string, val: unknown, schema: Schema): unknown => {
    ...
    if (type === "number") {
        const n = Number(val);
        if (!isNumber.test(n.toString())) throw invalidValue(key, val);
        return n;
    }
...
export type SchemaValidationFunction = <T>(
    val: T,
    key: string,
...
export interface Schema {
    validate?: boolean | RegExp | SchemaValidationFunction;
    type?: Schema;
    enum?: unknown[];
    required?: boolean;
    default?: unknown;
...
export default (
    ...
    if (fromEnvFile) {
        const dotenvPath =
            fromEnvFile === true ? resolve(process.cwd(), ".env") : fromEnvFile;
        ...
        if (schema.validate) {
            ...
            validationsToRun.push(() => {
                if (typeof schema.validate === "function") {
                    ...
                    if (!schema.validate.test(val.toString())) {
                        throw new Error(
                    ...
                    throw new Error(
                        `Invalid validate value for ${key} configuration`
                    );
                }
            });
        }
    ...
    return new Proxy(config, {
        get(target, prop) {
            if (
                !reservedSymbols.includes(prop.toString()) &&
                !hasOwnProperty(target, prop)
            ) {
                throw new Error(`Invalid config key "${prop.toString()}"`);
            }
